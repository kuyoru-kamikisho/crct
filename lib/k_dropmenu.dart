...
/// Defines a [KDropdownMenu] menu button that represents one item view in the menu.
///
/// See also:
///
/// * [KDropdownMenu]
class KDropdownMenuEntry<T> {
  /// Creates an entry that is used with [KDropdownMenu.dropdownMenuEntries].
  const KDropdownMenuEntry({
    required this.value,
    required this.label,
    this.labelWidget,
    this.leadingIcon,
  ...
  ///
  /// This value must be unique across all entries in a [KDropdownMenu].
  final T value;
  ...
  ///
  /// The default value is true. If true, the [KDropdownMenuEntry.label] will be filled
  /// out in the text field of the [KDropdownMenu] when this entry is clicked; otherwise,
...
///
/// The menu is composed of a list of [KDropdownMenuEntry]s. People can provide information,
...
///
/// {@tool dartpad}
/// This sample shows how to display outlined [KDropdownMenu] and filled [KDropdownMenu].
///
...
///   The [KDropdownMenu] uses a [TextField] as the "anchor".
/// * [TextField], which is a text input widget that uses an [InputDecoration].
/// * [KDropdownMenuEntry], which is used to build the [MenuItemButton] in the [KDropdownMenu] list.
class KDropdownMenu<T> extends StatefulWidget {
  /// Creates a const [KDropdownMenu].
  ///
  ...
  /// Determine if the [KDropdownMenu] is enabled.
  ///
  /// Defaults to true.
  final bool enabled;

  /// Determine the width of the [KDropdownMenu].
  ///
  /// If this is null, the width of the [KDropdownMenu] will be the same as the width of the widest
  ...
  /// Text that provides context about the [KDropdownMenu]'s value, such
  ...
  /// The text style for the [TextField] of the [KDropdownMenu];
  ///
  ...
  /// Descriptions of the menu items in the [KDropdownMenu].
  ///
  /// This is a required parameter. It is recommended that at least one [KDropdownMenuEntry]
  /// is provided. If this is an empty list, the menu will be empty and only
  /// contain space for padding.
  final List<KDropdownMenuEntry<T>> dropdownMenuEntries;
  ...
  /// by the width of menu items or [KDropdownMenu.width]. If this property is not null,
  ...
  @override
  State<KDropdownMenu<T>> createState() => _KDropdownMenuState<T>();
}

class _KDropdownMenuState<T> extends State<KDropdownMenu<T>> {
  ...
  late List<KDropdownMenuEntry<T>> filteredEntries;
  ...
  void initState() {
    ...
    _menuHasEnabledItem =
        filteredEntries.any((KDropdownMenuEntry<T> entry) => entry.enabled);

    final int index = filteredEntries.indexWhere(
        (KDropdownMenuEntry<T> entry) =>
  ...
  @override
  void didUpdateWidget(KDropdownMenu<T> oldWidget) {
  ...
  List<KDropdownMenuEntry<T>> filter(List<KDropdownMenuEntry<T>> entries,
      TextEditingController textEditingController) {
    final String filterText = textEditingController.text.toLowerCase();
    return entries
        .where((KDropdownMenuEntry<T> entry) =>
            entry.label.toLowerCase().contains(filterText))
        .toList();
  }

  int? search(List<KDropdownMenuEntry<T>> entries,
      TextEditingController textEditingController) {
    final String searchText = textEditingController.value.text.toLowerCase();
    if (searchText.isEmpty) {
      return null;
    }
    final int index = entries.indexWhere((KDropdownMenuEntry<T> entry) =>
        entry.label.toLowerCase().contains(searchText));

    return index != -1 ? index : null;
  }

  List<Widget> _buildButtons(List<KDropdownMenuEntry<T>> filteredEntries,
    ...
    for (int i = 0; i < filteredEntries.length; i++) {
      final KDropdownMenuEntry<T> entry = filteredEntries[i];
  ...
  Widget build(BuildContext context) {
    ...
    Widget menuAnchor = MenuAnchor(
      ...
      builder:
        ...
        final Widget trailingButton = SizedBox(
          width: 12,
          height: 12,
          child: IconButton(
        ...
        final Widget textField = TextField(
            ...
            onEditingComplete: () {
              if (currentHighlight != null) {
                final KDropdownMenuEntry<T> entry =
