...
// 发布流程：npm version -> npm run build -> npm publish -> git push --tag -> git push
...
const runRelease = (version) => {
  ...
  if (version) {
    ...
    execSync(`npm version ${version} -m "chore(release): %s"`);
    preVersion = `v${version}`;
  } else {
    const output = execSync('npm version patch -m "chore(release): %s"').toString('utf-8');
    preVersion = output.trim();
